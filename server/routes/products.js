...
router.post('/', requireAuth, requireAdmin, async (req, res) => {
  const { name, description, price, category, image_url, stock, details } = req.body;
  ...
  try {
    const { data: newProduct, error } = await supabase
      .from('products')
      .insert([{ name, description, price, category, image_url, stock, details }])
      .select()
      .single();
...
router.put('/:id', requireAuth, requireAdmin, async (req, res) => {
  ...
  try {
    const { data: updatedProduct, error } = await supabase
      .from('products')
      .update({ name, description, price, category, image_url, stock, details })
      .eq('id', req.params.id)
      .select()
      .single();
